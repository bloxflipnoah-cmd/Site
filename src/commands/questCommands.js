...
function buildAggregateProgressCard(total, completed) {
...
async function runQuestAll(userId, tokenStore, send) {
    ...
    try {
        ...
        // Single aggregate progress message
        let completedCount = 0;
        const total = valid.length;
        const progressMsg = await send(buildAggregateProgressCard(total, completedCount));

        const questLogs = valid.map(() => []);

        const taskPromises = valid.map((quest, i) => (async () => {
            const log = (m) => { console.log(m); questLogs[i].push(m); };
            try {
                const res = await manager.doingQuest(quest, log);
                if (res) {
                    completedCount++;
                    try { await progressMsg.edit(buildAggregateProgressCard(total, completedCount)); } catch {}
                }
        ...
        const skipped   = valid.filter((_, i) => results[i].status === 'rejected' || (results[i].status === 'fulfilled' && results[i].value === false));

        // Report failures (best-effort)
        if (skipped.length > 0) {
            const reasons = skipped.map((q) => {
                const idx = valid.indexOf(q);
                const failLogs = questLogs[idx].filter(l => l.startsWith('[FAIL]'));
                return `- ${q.config.messages.quest_name}: ${failLogs.slice(-2).join(' | ') || 'Could not be completed automatically.'}`;
            }).slice(0, 5).join('\n');
            try {
                const c = new ContainerBuilder().setAccentColor(0xED4245);
                c.addTextDisplayComponents(new TextDisplayBuilder().setContent(`# ⚠️ Some quests failed\n${reasons}`));
        ...
        try { await progressMsg.edit(buildAggregateProgressCard(total, total)); } catch {}

        const cDone = new ContainerBuilder().setAccentColor(0x57F287);
        cDone.addTextDisplayComponents(new TextDisplayBuilder().setContent(`# ✅ Auto-Quest Summary\n${completed.length} quest(s) completed automatically.\n${claimed > 0 ? `🎁 ${claimed} reward(s) claimed.` : 'No rewards claimed.'}`));
        await send({ components: [cDone], flags: MessageFlags.IsComponentsV2 });
        return true;
